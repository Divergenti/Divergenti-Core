...
const coin = { identity: 'diver', tooltip: 'Divergenti Core' }; // To simplify third party forks and different UIs for different coins, we'll define this constant that loads different assets.
...
ipcMain.on('start-daemon', (event, arg: Chain) => {
    ...
    if (arg.mode === 'manual') {
        ...
        const msg = 'Divergenti Core was started in development mode. This requires the user to be running the daemon manually.';
        writeLog(msg);
...
app.on('before-quit', () => {
    writeLog('Divergenti Core was exited.');
});
...
function startDaemon(chain: Chain) {
    ...
    if (chain.identity === 'diver') {
        daemonName = 'Divergenti.Node';
...
function launchDaemon(apiPath: string, chain: Chain) {
    ...
    daemonProcess.stdout.on('data', (data) => {
        writeDebug(`Divergenti Core: ${data}`);
    });

    /** Exit is triggered when the process exits. */
    daemonProcess.on('exit', function (code, signal) {
        writeLog(`Divergenti Core daemon process exited with code ${code} and signal ${signal} when the state was ${daemonState}.`);
        ...
        if (daemonState === DaemonState.Changing) {
            ...
            contents.send('daemon-error', `CRITICAL: Divergenti Core daemon process exited during startup with code ${code} and signal ${signal}.`);
        } else if (daemonState === DaemonState.Started) {
            contents.send('daemon-error', `Divergenti Core daemon process exited manually or crashed, with code ${code} and signal ${signal}.`);
        } else {
            // This is a normal shutdown scenario, but we'll show error dialog if the exit code was not 0 (OK).
            if (code !== 0) {
                contents.send('daemon-error', `Divergenti Core daemon shutdown completed, but resulted in exit code ${code} and signal ${signal}.`);
            } else {
    ...
    daemonProcess.on('error', (code, signal) => {
        writeError(`Divergenti Core daemon process failed to start. Code ${code} and signal ${signal}.`);
    });
}
...
function shutdownDaemon(callback) {
    ...
    if (!hasDaemon) {
        writeLog('Divergenti Core is in mobile mode, no daemon to shutdown.');
        callback(true, null);
